...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    const attempt = rateLimit.checkLoginAttempt(`account:${ip}`)
    if (!attempt.ok) {
    ...
    const sign = String(body.sign || body.seckey || '')
    const result = await account.loginOrRegisterUser(user, sign)
    if (!result) {
    ...
    rateLimit.clearLoginAttempts(`account:${ip}`)
